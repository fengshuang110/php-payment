...
if (class_exists('Library\Loader\AutoloaderFactory')) {
    return;
}

abstract class AutoloaderFactory
{
    const STANDARD_AUTOLOADER = 'Library\Loader\StandardAutoloader';
    ...
    public static function factory($options = null)
    {
        ...
        foreach ($options as $class => $autoloaderOptions) {
            if (!isset(static::$loaders[$class])) {
                ...
                if (!static::isSubclassOf($class, 'Library\Loader\SplAutoloader')) {
                    require_once 'Exception/InvalidArgumentException.php';
                    throw new Exception\InvalidArgumentException(
                        sprintf('Autoloader class %s must implement Library\\Loader\\SplAutoloader', $class)
                    );
                }
